---
_id: ea1dafce-8e5e-44a0-ad4c-b0c20eeabddc
category: Vue.js
topic: Watchers
difficulty: Hard
options:
...
::vueTip{:type="tip" :reference="https://vuejs.org/guide/essentials/watchers.html#watcheffect" :position="1"}
...
watchEffect(async () => {
  settings.value = await Promise.resolve({ showComments: false });
  // postId.value is accessed after an await, so changes to it won't trigger re-execution
  post.value = await getPost(postId.value);
});
```
